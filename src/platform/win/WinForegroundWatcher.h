...
// back.
class WinForegroundWatcher : public IForegroundWatcher {
public:
    ~WinForegroundWatcher() override { Stop(); }

    Support Availability() const override { return Support::Reliable; }

    // The callback runs on the calling thread, which must be the one pumping
    // messages — out-of-context hooks are delivered through the message
    // queue, and the debounce below is a window timer.
    bool Start(std::function<void(const ForegroundIdentity&)> onChange) override;
    void Stop() override;
    ...
    ForegroundIdentity Current() const override;
...
private:
    ...
    std::function<void(const ForegroundIdentity&)> m_onChange;
    ForegroundIdentity m_last;

    static WinForegroundWatcher* s_instance;
