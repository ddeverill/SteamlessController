...
WinForegroundWatcher* WinForegroundWatcher::s_instance = nullptr;
...
// pid is filled in regardless of whether the rest resolves, so a caller that
// only needs the process id (to map it to a Steam app, say) isn't blocked by
// AUMID/path lookups failing.
ForegroundIdentity IdentifyProcess(DWORD pid) {
    ForegroundIdentity id;
    if (pid == 0) return id;
    id.pid = static_cast<long>(pid);
    ...
    id.exePath = WideToUtf8(ImagePathOf(proc));
    id.appId   = WideToUtf8(AumidOf(proc));
    CloseHandle(proc);
    return id;
}
...
// Windows is the one platform where two spellings of a path can name the
// same file — fold case before comparing so a harmless case difference in
// what QueryFullProcessImageNameW returns doesn't read as a profile switch.
bool SameIdentity(const ForegroundIdentity& a, const ForegroundIdentity& b) {
    return ToLower(Utf8ToWide(a.exePath)) == ToLower(Utf8ToWide(b.exePath))
        && ToLower(Utf8ToWide(a.appId))   == ToLower(Utf8ToWide(b.appId));
}

}  // namespace

ForegroundIdentity WinForegroundWatcher::Current() const {
    ...
    if (!id.exePath.empty() && IsFrameHost(Utf8ToWide(id.exePath))) {
...
void CALLBACK WinForegroundWatcher::HookProc(HWINEVENTHOOK, DWORD event, HWND hwnd,
...
LRESULT CALLBACK WinForegroundWatcher::WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
...
void WinForegroundWatcher::OnDebounceElapsed() {
    ...
    // cannot open.
    if (now.Empty() || SameIdentity(now, m_last)) return;
    m_last = now;
    if (m_onChange) m_onChange(now);
}

bool WinForegroundWatcher::Start(std::function<void(const ForegroundIdentity&)> onChange) {
    Stop();
...
void WinForegroundWatcher::Stop() {
    if (m_hook) {
