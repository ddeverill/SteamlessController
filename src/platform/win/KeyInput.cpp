...
// The physical key each modifier flag names, for display purposes only (the
// actual press/release refcounting this table used to drive now lives in
// core/ModifierTracker.h, shared with the Linux CLI's binding display).
struct ModifierKey { BackButtonBinding::Modifier flag; const wchar_t* name; };

// Order matters: it is the order a label reads in. Ctrl, Alt, Shift, Win is
// the convention Windows itself uses when it spells out a shortcut.
const ModifierKey kModifierKeys[] = {
    { BackButtonBinding::ModCtrl,  L"Ctrl"  },
    { BackButtonBinding::ModAlt,   L"Alt"   },
    { BackButtonBinding::ModShift, L"Shift" },
    { BackButtonBinding::ModWin,   L"Win"   },
};
