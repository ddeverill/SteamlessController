...
#include "core/Text.h"
// ViGEm/Common.h uses USHORT/BYTE/SHORT/... without defining them itself —
// it assumes Windows.h is already included. That used to arrive for free
// through SteamController.h's old HidDevice.h include; the cross-platform
// port replaced that with the platform-neutral IHidDevice.h, so it has to be
// brought in explicitly here now.
...
{
    if (userData)
        static_cast<WinVirtualGamepad*>(userData)->OnRumble(largeMotor, smallMotor);
}

// Keys and mouse buttons are delivered by ControllerManager via the input
// injector, not through the virtual pad, so they fall through as "no gamepad
// input".
...
WinVirtualGamepad::WinVirtualGamepad(ControllerPlatform platform, RumbleCallback rumbleCallback)
    : m_platform(platform), m_rumbleCallback(std::move(rumbleCallback))
{
    ...
    if (!VIGEM_SUCCESS(err)) {
        ...
        m_busReport     = WideToUtf8(ViGEmBusInfo::Describe(buses));
...
WinVirtualGamepad::~WinVirtualGamepad() {
    StopDs4OutputThread();
...
void WinVirtualGamepad::StartDs4OutputThread() {
    if (m_ds4OutputRunning.exchange(true)) return;
    m_ds4OutputThread = std::thread(&WinVirtualGamepad::Ds4OutputLoop, this);
}

void WinVirtualGamepad::StopDs4OutputThread() {
    m_ds4OutputRunning = false;
    if (m_ds4OutputThread.joinable())
        m_ds4OutputThread.join();
}

void WinVirtualGamepad::Ds4OutputLoop() {
...
void WinVirtualGamepad::SetBatteryState(uint8_t levelPercent, uint8_t chargeState) {
...
void WinVirtualGamepad::Update(const uint8_t* buf, size_t n,
...
void WinVirtualGamepad::OnRumble(uint8_t largeMotor, uint8_t smallMotor) {
    if (m_rumbleCallback)
        m_rumbleCallback(largeMotor, smallMotor);
}

// ---------------------------------------------------------------------------
// WinVirtualGamepadFactory
// ---------------------------------------------------------------------------

std::unique_ptr<IVirtualGamepad> WinVirtualGamepadFactory::Create(
        ControllerPlatform platform, RumbleCallback rumbleCallback) {
    return std::make_unique<WinVirtualGamepad>(platform, std::move(rumbleCallback));
}

bool WinVirtualGamepadFactory::BusAvailable() const {
    return !ViGEmBusInfo::Enumerate().empty();
}

std::string WinVirtualGamepadFactory::BusReport() const {
    return WideToUtf8(ViGEmBusInfo::Describe(ViGEmBusInfo::Enumerate()));
}
