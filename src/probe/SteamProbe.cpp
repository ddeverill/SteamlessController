...
// ---------------------------------------------------------------------------
// Transport identification — bus type comes straight from the backend now
// (HidDeviceInfo::bus), rather than sniffing the device path for markers.
// ---------------------------------------------------------------------------

static const char* GuessTransport(uint16_t pid, HidBus bus) {
    HidDeviceInfo info;
    info.pid = pid;
    info.bus = bus;
    return SteamController::TransportName(SteamController::TransportFrom(info));
}
...
static std::vector<IfaceInfo> EnumerateAllValveDevices() {
    ...
    WinHidBackend backend;
    auto devices = backend.Enumerate(SteamController::VALVE_VID, 0);
    ...
    for (auto const& device : devices) {
        IfaceInfo info;
        info.path = Utf8ToWide(device.path);
        info.bus  = device.bus;
        ...
        if (h == INVALID_HANDLE_VALUE) {
            printf("  [%zu] (open failed, error %lu)  %s\n      path: %ls\n",
                   out.size(), GetLastError(), GuessTransport(device.pid, info.bus), path.c_str());
        ...
        printf("  [%zu]%s PID=%04X  Usage=%04X:%04X  In=%u Out=%u Feat=%u  fw=0x%04X  %s\n"
               ...
               attrs.VersionNumber, GuessTransport(info.pid, info.bus),
...
int main(int argc, char** argv) {
    ...
    size_t chosen = ifaces.size() > 1
        ...
        : defaultIdx;
    printf("Opening [%zu] (%s)...\n\n", chosen, GuessTransport(ifaces[chosen].pid, ifaces[chosen].bus));

    if (argc > 1 && strcmp(argv[1], "--enumerate-only") == 0)
        return 0;

    WinHidBackend backend;
    SteamController controller(backend);
    g_controller = &controller;

    if (!controller.Open(WideToUtf8(ifaces[chosen].path))) {
