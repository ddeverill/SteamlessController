...
#include "platform/win/GameProfiles.h"
...
bool TrayApp::Init(HINSTANCE hInstance) {
    ...
    // Built before anything below logs or touches the controller:
    // ControllerManager's constructor takes IPlatform& now (shared with the
    // Linux daemon's ControllerManager use), and EventLog needs a directory
    // handed to it explicitly rather than computing %LOCALAPPDATA% itself.
    m_platform = std::make_unique<WinPlatform>();
    EventLog::Init(m_platform->Paths().StateDir());
    ...
    // ControllerManager::AlertFn (and WinInputInjector::AlertFn) are UTF-8
    // std::string now, shared with the Linux side — converted to wstring
    // here, at the one place this app still deals in wide strings for its
    // own UI (ShowAlertBalloon).
    auto raiseAlert = [this](const std::string& title, const std::string& text) {
        {
            std::lock_guard<std::mutex> lk(m_alertMutex);
            m_alertTitle = Utf8ToWide(title);
            m_alertText  = Utf8ToWide(text);
        }
    ...
    m_platform->GetInput().SetAlertCallback(raiseAlert);
...
std::wstring TrayApp::MatchProfile(const ForegroundIdentity& id) const {
    ...
    // ForegroundIdentity is UTF-8 std::string now (shared with the Linux
    // side, whose foreground-detection backends have no concept of "wide
    // strings" at all) — converted to wstring here, once, since every game
    // profile below this point is still keyed by the wstring ids
    // GameProfiles/RegKey persistence has always used.
    const std::wstring exePath = Utf8ToWide(id.exePath);
    const std::wstring appId   = Utf8ToWide(id.appId);
    ...
    const std::wstring steamAppId = Utf8ToWide(m_steamApps.AppIdForPath(id.exePath));

    for (const auto& [profileId, profile] : m_gameProfiles) {
        if (profileId.rfind(L"aumid:", 0) == 0) {
            if (!appId.empty()
                && _wcsicmp(profileId.c_str() + 6, appId.c_str()) == 0)
                return profileId;
            ...
            if (!steamAppId.empty() && profileId.compare(kSteamPrefixLen,
                                                         ...
                                                         steamAppId) == 0)
                return profileId;
        } else if (SamePath(profileId, exePath)) {
            return profileId;
        }
    }
    return {};
...
void TrayApp::RefreshSteamApps() {
    // Re-resolved from the registry every time, same as the pre-port
    // SteamAppLocator did internally — a Steam reinstall to a new drive is
    // picked up without restarting this app.
    m_steamApps.SetRoots(m_platform->Paths().SteamRoots());
    m_steamApps.Refresh();
...
void TrayApp::ActivateProfile(const std::wstring& gameId) {
    ...
    const std::wstring name = profile->displayName.empty() ? gameId
                                                           : Utf8ToWide(profile->displayName);
...
void TrayApp::RefreshActiveProfile() {
    ...
    if (m_remapWindow.IsOpen()) return;
    ActivateProfile(MatchProfile(m_foregroundWatcher.Current()));
}
...
bool TrayApp::RestartControllerDevices() {
    ...
    if (IsProcessElevated()) {
        ...
        for (const auto& info : SteamController::EnumerateAll(m_platform->GetHid())) {
            DeviceRestart::CycleResult r;
            if (DeviceRestart::RestartInterfaceDevice(Utf8ToWide(info.path), r))
                anyRestarted = true;
...
void TrayApp::OpenEventLog() {
    ...
    ShellExecuteW(nullptr, L"open", L"notepad.exe",
                  Utf8ToWide(EventLog::FilePath()).c_str(), nullptr, SW_SHOWNORMAL);
}
