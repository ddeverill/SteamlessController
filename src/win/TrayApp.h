...
#include "platform/win/WinSteamWatcher.h"
...
private:
    ...
    // Constructed before m_controller: ControllerManager takes IPlatform& in
    // its constructor now, shared with the Linux daemon's ControllerManager
    // use (see core/ControllerManager.h). Nothing else here is routed
    // through it yet — see WinPlatform's class comment.
    std::unique_ptr<WinPlatform>       m_platform;
    std::unique_ptr<ControllerManager> m_controller;
    SteamWatcher                       m_steamWatcher;
    WinForegroundWatcher               m_foregroundWatcher;
    ...
    // Windows filesystems fold case, so caseInsensitivePaths=true.
    SteamAppLocator                    m_steamApps{true};
